...
int main(void)
{
    ...
    while (!WindowShouldClose())    // detectar el boton de cierre de la ventana o la tecla ESC
    {
...
void ActualizarJuego(void)
{
    if (!juegoTerminado)
    {
        ...
        // colision con los límites de la matriz
    ...
    else
    {
        ...
        {
            LiberarSerpiente(&serpiente); // libera la memoria de la serpiente antes de reiniciar el juego
            InicializarJuego();
            juegoTerminado = false;
        }
    }
